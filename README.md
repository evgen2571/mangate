...
The TUI is a sequential keyboard flow: search, choose a title, select one or more chapters, choose Directory, CBZ, or ZIP, then review the operation before it starts. The review shows known page totals, every planned output path, existing-output policy decisions, and whether source pages are retained after archive validation. A final result screen reports the outcome and output paths. Use arrows or `j` and `k` to move, `/` to filter chapters by label, language, or ID, `space` to toggle chapters, `a` to select all visible chapters, `d` to clear them, `enter` to continue, and `esc` to go back. `ctrl+g` opens configuration, including output root, default format, existing-file policy, and source retention; applying it while reviewing immediately refreshes the plan. It has no mouse requirement. `ctrl+c` or `q` exits before a download begins.
