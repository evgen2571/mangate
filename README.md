...
The terminal UI is deliberately plain: a single keyboard flow for search, title selection, chapter selection, output format and directory, review, download progress, and completion. Use arrows or `j` and `k` to move, `space` to toggle a chapter, `a` to select visible chapters, `l` for the latest visible chapter, `r` to extend the current range, `d` to clear, and `/` to filter chapters. `f` from search results selects every chapter. `ctrl+g` opens the editable configuration. `ctrl+c` or `q` exits before downloading, or cancels an active download while keeping completed page data.
